...
export function Seq() {
  ...
  const frameReadoutRef = useRef<HTMLSpanElement | null>(null);
  ...
  useEffect(() => {
    ...
    for (let i = 1; i <= YOUR_FRAME_COUNT; i++) {
      const img = new Image();
      img.src = framePath(i);
      img.onload = () => {
        if (cancelled) return;
        loadedCount++;
        setLoadProgress(loadedCount / YOUR_FRAME_COUNT);
        if (loadedCount === YOUR_FRAME_COUNT) {
    ...
    const handleScroll = () => {
      ...
      requestAnimationFrame(() => {
        ...
        const frameIndex = Math.min(
          YOUR_FRAME_COUNT - 1,
          Math.floor(progress * YOUR_FRAME_COUNT),
        );
        ...
        if (heroTextRef.current) {
          const opacity = Math.max(0, 1 - progress / YOUR_TEXT_FADE_END);
        ...
        if (frameReadoutRef.current) {
          frameReadoutRef.current.textContent = `SEQ ${String(frameIndex + 1).padStart(3, "0")} / ${YOUR_FRAME_COUNT}`;
        }

        // Show/hide quote cards
        const newVisible = new Set<string>();
        for (const d of YOUR_DIALOGUES) {
  ...
  return (
    <section ref={sectionRef} className="scroll-animation relative">
      ...
      >
        ...
        {/* Gradient overlay - warm artisan tones */}
        <div
        ...
        >
          ...
          <p className="max-w-[42ch] font-sans text-sm leading-relaxed text-zinc-400 md:text-base">
            Each piece is a limited edition — not by design, but by the nature of handwork. 
            When it's gone, it's truly gone. Made to last generations.
          </p>
        </div>

        {/* Left side label - Materials */}
        <div className="pointer-events-none absolute left-6 top-20 z-10 flex items-center gap-2 md:left-10 md:top-24">
          ...
          <span className="font-mono text-[10px] uppercase tracking-[0.32em] text-zinc-400">
            Materials &mdash; Ethically Sourced
          </span>
        </div>

        {/* Right side telemetry - Sustainability */}
        ...
        <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10">
          ...
          <div className="mx-6 flex items-center justify-between pb-4 font-mono text-[10px] uppercase tracking-[0.28em] text-zinc-500 md:mx-10">
            ...
            <span>HANDMADE // HEIRLOOM</span>
            <span>Scroll &darr;</span>
          </div>
        </div>

        {/* Desktop Quote Cards - Craftsmanship */}
        {YOUR_DIALOGUES.map((d, i) => {
        ...
        {!loaded && (
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-center gap-5 bg-background px-6">
            <EyebrowBadge>ARTISAN LOOP // CURATING COLLECTION</EyebrowBadge>
            ...
            <p className="font-mono text-[11px] uppercase tracking-[0.28em] text-zinc-500">
              Preparing heirloom pieces &nbsp;&middot;&nbsp; {Math.round(loadProgress * 100)}%
            </p>
          </div>
        )}
      </div>
